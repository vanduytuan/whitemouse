...
if ($_REQUEST['filter']) {
	...
	if(!is_numeric($dr_start) || $dr_start <= 0) $dr_start = 0;
	$dr_end = $_REQUEST['dr_end'];
	if(!is_numeric($dr_end) || $dr_end >= 50) $dr_end = 50;
...
if (isset($dr_start) && isset($dr_end)){$depth = " and c.sd_evn_edep BETWEEN '$dr_start' AND '$dr_end' ";}
if (isset($eqtype)){$quaketype = " and sd_evn_eqtype = '$eqtype' ";}
...
if(document.getElementById){
	 $(document).ready(function createMap(divId,showStations){
		...
		google.maps.event.addListener(marker,'click',function(){
			...
			var newIcon=new google.maps.MarkerImage('../img/pin_ds_s.png',new google.maps.Size(15,23),null,null);
			...
			if(<?php echo ""+$staplot;?>==9){
					...
					<?php
						...
						echo "var newIcon=new google.maps.MarkerImage('../img/pin_ss_s.png',new google.maps.Size(15,23),null,null);";
					...
					// Deformation stations: PURPLE markers
						echo "var newIcon=new google.maps.MarkerImage('../img/pin_ds_s.png',new google.maps.Size(15,23),null,null);";
						...
						echo "var newIcon=new google.maps.MarkerImage('../img/pin_hs_s.png',new google.maps.Size(15,23),null,null);";
						...
						echo "var newIcon=new google.maps.MarkerImage('../img/pin_ts_s.png',new google.maps.Size(15,23),null,null);";
						...
						echo "var newIcon=new google.maps.MarkerImage('../img/pin_gs_s.png',new google.maps.Size(15,23),null,null);";
						...
						echo "var newIcon=new google.maps.MarkerImage('../img/pin_fs_s.png',new google.maps.Size(15,23),null,null);";
...
//							$getQuakes = mysql_query("(select b.sn_code, c.sd_evn_elat, c.sd_evn_elon, c.sd_evn_edep, c.sd_evn_pmag, c.sd_evn_time, c.sd_evn_eqtype, d.vd_inf_slat, d.vd_inf_slon FROM sn b, sd_evn c, vd_inf d WHERE b.sn_id = c.sn_id AND b.vd_id=d.vd_id AND d.vd_id = '$vd_id' $dates $depth $quaketype ORDER BY (sd_evn_time) DESC limit $qty) UNION (select b.sn_code, c.sd_evn_elat, c.sd_evn_elon, c.sd_evn_edep, c.sd_evn_pmag, c.sd_evn_time, c.sd_evn_eqtype, d.vd_inf_slat, d.vd_inf_slon	FROM jj_volnet a, sn b, sd_evn c, vd_inf d WHERE a.vd_id = '$vd_id' AND a.jj_net_id = b.sn_id AND b.sn_id = c.sn_id	AND d.vd_id = '$vd_id' AND a.jj_net_flag = 'S' $dates $depth $quaketype AND (sqrt(power(d.vd_inf_slat - c.sd_evn_elat, 2) + power(d.vd_inf_slon - c.sd_evn_elon, 2))*100)<=10 ORDER BY (sd_evn_time) DESC limit $qty)") or die(mysql_error());
							...
							$end = split('/', $endDate);
							$dates2 = " and sd_evn_time BETWEEN '$start[2]-$start[0]-$start[1]' AND '$end[2]-$end[0]-$end[1]' ";
							$depth2 = " and sd_evn_edep BETWEEN '$dr_start' AND '$dr_end'";	
							if($eqType != '') $equakeType2 = " and sd_evn_eqtype = '$eqType' ";
							$getQuakes = mysql_query("(select sd_evn_elat, sd_evn_elon, sd_evn_edep, sd_evn_pmag, sd_evn_time, sd_evn_eqtype, sn_id FROM sd_evn WHERE ($vd_info_obj->vd_inf_slat - sd_evn_elat)<1 and ($vd_info_obj->vd_inf_slon - sd_evn_elon)<6 and (sqrt(pow(($vd_info_obj->vd_inf_slat - sd_evn_elat)*110, 2) + pow(($vd_info_obj->vd_inf_slon - sd_evn_elon)*111.32*cos(($vd_info_obj->vd_inf_slat)/57.32), 2)))<30 LIMIT $qty)") or die(mysql_error());

							$count1 = 1;
							$clear = "";
							echo "var recentWindow = new google.maps.InfoWindow({content:\"\",});";
							while ($getQuake_obj = mysql_fetch_object($getQuakes)){
								...
								$mag = round($getQuake_obj->sd_evn_pmag)*2;
								if ($mag<2) $mag = 2;
								...
								echo "point = new google.maps.LatLng($getQuake_obj->sd_evn_elat,$getQuake_obj->sd_evn_elon);
									newIcon$md5=new google.maps.MarkerImage($color,null,null,null,new google.maps.Size($mag,$mag));
									";
									
								$clear .= "window.str$md5 = '';\n";
								
								echo "if (window.str$md5 === undefined) { str$md5 = '$count1. Time: $getQuake_obj->sd_evn_time\\nDepth:$getQuake_obj->sd_evn_edep , Mag:$getQuake_obj->sd_evn_pmag\\nType:$getQuake_obj->sd_evn_eqtype ,Netw: $getQuake_obj->sn_id'; }
										else { str$md5 += '\\n$count1. Time:$getQuake_obj->sd_evn_time\\nDepth: $getQuake_obj->sd_evn_edep , Mag:$getQuake_obj->sd_evn_pmag\\nType: $getQuake_obj->sd_evn_eqtype ,Netw: $getQuake_obj->sn_id' ;}
										st2$md5 = str$md5;
										marker$md5 = new google.maps.Marker({position:point,map:map,icon: newIcon$md5});
...
//				$neighbors = mysql_query("select a.vd_id, b.vd_name, a.vd_inf_slat, a.vd_inf_slon, (pow(a.vd_inf_slat - $vd_info_obj->vd_inf_slat, 2) + pow(a.vd_inf_slon - $vd_info_obj->vd_inf_slon, 2)) as `distance` FROM vd_inf a, vd b WHERE a.vd_id != '$vd_id' AND a.vd_id = b.vd_id and SUBSTR(b.vd_name,1,7)!='$unn' and a.vd_inf_status!='$statusUnc' and a.vd_inf_status!='$statusNot' and a.vd_inf_status!='$statusUr' and a.vd_inf_type!='$typeSubm' ORDER BY `distance` ASC limit 15") or die(mysql_error());				
				$neighbors = mysql_query("select a.vd_id, b.vd_name, a.vd_inf_slat, a.vd_inf_slon FROM vd_inf a, vd b WHERE a.vd_id != '$vd_id' AND a.vd_id = b.vd_id and SUBSTR(b.vd_name,1,7)!='$unn' and a.vd_inf_status!='$statusUnc' and a.vd_inf_status!='$statusNot' and a.vd_inf_status!='$statusUr' and a.vd_inf_type!='$typeSubm' ORDER by (pow(a.vd_inf_slat - $vd_info_obj->vd_inf_slat, 2) + pow(a.vd_inf_slon - $vd_info_obj->vd_inf_slon, 2)) limit 15") or die(mysql_error());				
				...
				$vd_name_n=addslashes($neighbor_obj->vd_name);
					if($count<12)
					{
						echo "var newIcon= new google.maps.MarkerImage('../img/pin.png',new google.maps.Size(12,12),null,null);
