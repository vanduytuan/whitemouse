...
<?php
	...
	$vdi=$_GET["vdi"];
	$result = mysql_query("select a.vd_name, b.vd_inf_status, b.vd_inf_type, c.cc_url 	FROM vd a, vd_inf b, cc c WHERE a.vd_id='$vdi' and a.vd_id=b.vd_id and a.cc_id=c.cc_id");
	...
	$result2 = mysql_query(" select c.ed_stime FROM vd a, ed c WHERE a.vd_id='$vdi' and a.vd_id=c.vd_id ORDER by c.ed_stime DESC");
