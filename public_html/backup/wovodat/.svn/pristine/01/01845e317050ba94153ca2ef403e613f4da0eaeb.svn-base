...
// ^^^ Get owners
if (!v1_get_owners($da_fd_gra_obj, $error)) {
...
// ^^^ Get instrument
v1_get_ms($da_fd_gra_obj, "INSTRUMENT", $gen_instruments);

// ^^^ Get station
v1_get_ms($da_fd_gra_obj, "STATION", $gen_stations);

// ^^^ Get reference station
v1_get_ms($da_fd_gra_obj, "REFSTATION", $gen_stations2);

// ^^^ Get publish date
v1_get_pubdate($da_fd_gra_obj);
