	  ...
	  if (showStations) 
	  {
		...
		while ($getStation_obj = mysql_fetch_object($getStations))
		{
			...
			GEvent.addListener(marker$ctr, 'click', function() {
			  marker$ctr.openInfoWindowHtml('Deformation station: $getStation_obj->ds_name<br>Lat: $getStation_obj->ds_nlat<br>Lon: $getStation_obj->ds_nlon');
		...
		$getStations = mysql_query("select c.sn_id, c.sd_evn_elat, c.sd_evn_elon, 
										c.sd_evn_edep, c.sd_evn_pmag, c.sd_evn_time, 
										c.sd_evn_eqtype, d.vd_inf_slat, d.vd_inf_slon 
										from sn a, sd_evn c, vd_inf d 
										where a.vd_id='$vd_id' and a.sn_id=c.sn_id
										and d.vd_id = '$vd_id' 
