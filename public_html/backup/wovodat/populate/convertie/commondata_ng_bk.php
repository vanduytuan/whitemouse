...
if($conv=="DirectlySampledGas" || $conv == 'SoilEffluxData' || $conv == 'PlumeData'){
	...
	if(isset($airplane)){
		for($j=0;$j<sizeof($code);$j++){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\"$owner2[$j]$owner3[$j]$pubdate[$j]";
		}
	}
	else if(isset($satellite)){
		for($j=0;$j<sizeof($code);$j++){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" satellite=\"$air_satellite\" owner1=\"$observ\"$owner2[$j]$owner3[$j]$pubdate[$j]";
		}	
	}
}
...
//Upload Two csv files can't have more than one "rowhead <ThermalImage code="123" owner1="CVGHM"....>". Bcoz Thermalpixels child can't distinguish more than one parent.. So ThermalImage csv file always must have one row.
	...
	if(isset($airplane)){
		$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\"$owner2[0]$owner3[0]$pubdate[0]";
	}else if(isset($satellite)){
		$rowhead= "ThermalImage code=\"$code[0]\" satellite=\"$air_satellite\" owner1=\"$observ\"$owner2[0]$owner3[0]$pubdate[0]";
	}else{
		$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\"$owner2[0]$owner3[0]$pubdate[0]";
	}
/*
	if(isset($airplane)){
		if((isset($owner2)) && (isset($owner3))){
