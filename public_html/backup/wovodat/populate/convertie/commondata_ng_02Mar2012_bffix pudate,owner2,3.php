...
$rowprefix = "";	
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" network=\"$network\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" network=\"$network\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" network=\"$network\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" network=\"$network\" owner1=\"$observ\"";	
		}	
...
$rowprefix = "";
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" station=\"$station\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" station=\"$station\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" station=\"$station\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" station=\"$station\" owner1=\"$observ\"";	
		}	
...
if($conv=="ElectronicTiltData" || $conv == "TiltVectorData" || $conv == "StrainMeterData" || $conv == "GPSVectors"){
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\"  instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\"";	
		}	
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\"  instrument=\"$instrument\" station=\"$station\" targetStation=\"$station2\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" targetStation=\"$station2\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" targetStation=\"$station2\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" targetStation=\"$station2\" owner1=\"$observ\"";	
		}	
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\"  instrument=\"$instrument\" station=\"$station\" targetStation1=\"$station2\" targetStation2=\"$station3\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" targetStation1=\"$station2\" targetStation2=\"$station3\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" targetStation1=\"$station2\" targetStation2=\"$station3\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" targetStation1=\"$station2\" targetStation2=\"$station3\" owner1=\"$observ\"";	
		}	
	}
}
else if($conv == 'GPSData'){
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\"  instrument=\"$instrument\" station=\"$station\" refStation1=\"$station2\" refStation2=\"$station3\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" refStation1=\"$station2\" refStation2=\"$station3\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" refStation1=\"$station2\" refStation2=\"$station3\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" refStation1=\"$station2\" refStation2=\"$station3\" owner1=\"$observ\"";	
		}	
	}
}
...
if($conv=="DirectlySampledGas" || $conv == 'SoilEffluxData' || $conv == 'PlumeData'){
	...
	if(isset($airplane)){
		for($j=0;$j<sizeof($code);$j++){
			if((isset($owner2)) && (isset($owner3))){
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\" $owner2 $owner3";
			}else if(isset($owner2)){
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\" $owner2";	
			}else if(isset($owner3)){
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" airplane=\"$air_satellite\"  owner1=\"$observ\" $owner3";	
			}else{
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\"";	
			}	
		}
		...
		for($j=0;$j<sizeof($code);$j++){
			if((isset($owner2)) && (isset($owner3))){
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" satellite=\"$air_satellite\" owner1=\"$observ\" $owner2 $owner3";
			}else if(isset($owner2)){
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" satellite=\"$air_satellite\" owner1=\"$observ\" $owner2";	
			}else if(isset($owner3)){
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" satellite=\"$air_satellite\"  owner1=\"$observ\" $owner3";	
			}else{
				$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" satellite=\"$air_satellite\" owner1=\"$observ\"";	
			}	
		}	
	}
}
...
HTMLBLOCK;
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "HydrologicSample code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "HydrologicSample code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "HydrologicSample code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\"  owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "HydrologicSample code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\"";	
		}	
...
if($conv == 'MagnetorVectorData'){
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\"  instrument=\"$instrument\" station=\"$station\" refStation=\"$station2\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" refStation=\"$station2\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" refStation=\"$station2\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" station=\"$station\" refStation=\"$station2\" owner1=\"$observ\"";	
		}	
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\"  instrument=\"$instrument\" refStation1=\"$station\" refStation2=\"$station2\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" refStation1=\"$station\" refStation2=\"$station2\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" refStation1=\"$station\" refStation2=\"$station2\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "$xmlinitag code=\"$code[$j]\" instrument=\"$instrument\" refStation1=\"$station\" refStation2=\"$station2\" owner1=\"$observ\"";	
		}	
...
//Upload Two csv files can't have more than one "rowhead <ThermalImage code="123" owner1="CVGHM"....>". Bcoz Thermalpixels child can't distinguish more than one parent.. So ThermalImage csv file always must have one row.
	...
	if(isset($airplane)){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" airplane=\"$air_satellite\" owner1=\"$observ\"";	
		}	
	}else if(isset($satellite)){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead= "ThermalImage code=\"$code[0]\" satellite=\"$air_satellite\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead= "ThermalImage code=\"$code[0]\" satellite=\"$air_satellite\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead= "ThermalImage code=\"$code[0]\" satellite=\"$air_satellite\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead= "ThermalImage code=\"$code[0]\" satellite=\"$air_satellite\" owner1=\"$observ\"";
		}	
	}else{
		if((isset($owner2)) && (isset($owner3))){
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead= "ThermalImage code=\"$code[0]\" instrument=\"$instrument\" station=\"$station\" owner1=\"$observ\"";	
		}
	}
...
//Upload Two csv files can't have more than one "rowhead <InSARImage code="123" owner1="CVGHM"....>". Bcoz InSARpixels child can't distinguish more than one parent.. So InSARImage csv file always must have one row.
	...
	if((isset($owner2)) && (isset($owner3))){
		$rowhead= "InSARImage code=\"$code[0]\" satellite=\"$satellite\" owner1=\"$observ\" $owner2 $owner3";
	}else if(isset($owner2)){
		$rowhead= "InSARImage code=\"$code[0]\" satellite=\"$satellite\" owner1=\"$observ\" $owner2";	
	}else if(isset($owner3)){
		$rowhead= "InSARImage code=\"$code[0]\" satellite=\"$satellite\" owner1=\"$observ\" $owner3";	
	}else{
		$rowhead= "InSARImage code=\"$code[0]\" satellite=\"$satellite\" owner1=\"$observ\"";	
	}	
...
$rowprefix = "";	
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "Leveling code=\"$code[$j]\" $instrument[$j] $refStation[$j] $firstBMStation[$j] $secondBMStation[$j] owner1=\"$observ\" $owner2 $owner3";		

		}else if(isset($owner2)){
			$rowhead[$j+1]= "Leveling code=\"$code[$j]\" $instrument[$j] $refStation[$j] $firstBMStation[$j] $secondBMStation[$j] owner1=\"$observ\" $owner2";		

		}else if(isset($owner3)){
			$rowhead[$j+1]= "Leveling code=\"$code[$j]\" $instrument[$j] $refStation[$j] $firstBMStation[$j] $secondBMStation[$j] owner1=\"$observ\"$owner3";					
		
		}else{
			$rowhead[$j+1]= "Leveling code=\"$code[$j]\" $instrument[$j] $refStation[$j] $firstBMStation[$j] $secondBMStation[$j] owner1=\"$observ\"";				
		}	
...
$rowprefix = "";
	...
	for($j=0;$j<sizeof($code);$j++){
		...
		if(isset($singleStationEvent[$j])){	
			...
			if((isset($owner2)) && (isset($owner3))){
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $singleStationEvent[$j] owner1=\"$observ\" $owner2 $owner3";
			}else if(isset($owner2)){
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $singleStationEvent[$j] owner1=\"$observ\" $owner2";	
			}else if(isset($owner3)){
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $singleStationEvent[$j] owner1=\"$observ\" $owner3";	
			}else{
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $singleStationEvent[$j] owner1=\"$observ\"";	
			}	
		}
		else{
			if((isset($owner2)) && (isset($owner3))){
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $networkEvent[$j] owner1=\"$observ\" $owner2 $owner3";
			}else if(isset($owner2)){
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $networkEvent[$j] owner1=\"$observ\" $owner2";	
			}else if(isset($owner3)){
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $networkEvent[$j] owner1=\"$observ\" $owner3";	
			}else{
				$rowhead[$j+1]= "Intensity code=\"$code[$j]\" $networkEvent[$j] owner1=\"$observ\"";	
			}	
...
$rowprefix = "";	
	...
	for($j=0;$j<sizeof($code);$j++){
		if((isset($owner2)) && (isset($owner3))){
			$rowhead[$j+1]= "Waveform code=\"$code[$j]\" $networkEvent$singleStationEvent$tremor station=\"$station\" owner1=\"$observ\" $owner2 $owner3";
		}else if(isset($owner2)){
			$rowhead[$j+1]= "Waveform code=\"$code[$j]\" $networkEvent $singleStationEvent $tremor station=\"$station\" owner1=\"$observ\" $owner2";	
		}else if(isset($owner3)){
			$rowhead[$j+1]= "Waveform code=\"$code[$j]\" $networkEvent $singleStationEvent $tremor station=\"$station\" owner1=\"$observ\" $owner3";	
		}else{
			$rowhead[$j+1]= "Waveform code=\"$code[$j]\" $networkEvent $singleStationEvent $tremor station=\"$station\" owner1=\"$observ\"";	
		}	
	}
