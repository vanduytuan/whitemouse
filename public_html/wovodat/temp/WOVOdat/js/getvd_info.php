...
?>  <script type="text/javascript">
    ...
    if (GBrowserIsCompatible()) { 
      ...
      var marker = createMarker(point,'<div><?php echo "Current Volcano: " . addslashes($vd_name) . "<br>Lat: $vd_info_obj->vd_inf_slat<br>$vd_info_obj->vd_inf_slon"; ?><\/div>');
	  ...
	  if (showStations) 
	  {
		...
		<?php
		$ctr = 0;
		/*//$getStationNetwork = mysql_query("select sn_id from sn where vd_id = '$vd_id'") or die(mysql_error());
		//if ($getStations_obj = mysql_fetch_object($getStationNetwork))
		//{
			//$getStations = mysql_query("select ss_lat, ss_lon, ss_name from ss where sn_id = '$getStations_obj->sn_id'") or die(mysql_error());
			$getStations = mysql_query("select a.sn_id, b.ss_lat, b.ss_lon, b.ss_name from sn a, ss b where a.vd_id = '$vd_id' and b.sn_id = a.sn_id") or die(mysql_error());
			$output = "GPS Stations : " . mysql_num_rows($getStations);
			while ($getStation_obj = mysql_fetch_object($getStations))
			{
				...
				GEvent.addListener(marker$ctr, 'click', function() {
				  marker$ctr.openInfoWindowHtml('$getStation_obj->ss_name');
		...
		while ($getStation_obj = mysql_fetch_object($getStations))
		{
			...
			echo "var marker$ctr = new GMarker(point, {icon: newIcon});
			GEvent.addListener(marker$ctr, 'click', function() {
			  marker$ctr.openInfoWindowHtml('GPS station: $getStation_obj->gs_name<br>Lat: $getStation_obj->gs_lat<br>Lon: $getStation_obj->gs_lon');
			}); map.addOverlay(marker$ctr);";
			$ctr++;
		}
		...
		while ($getStation_obj = mysql_fetch_object($getStations))
		{
			if ($getStation_obj->sd_evn_edep <= 2.5) $color = "#FF0000";
			else if ($getStation_obj->sd_evn_edep <= 5) $color = "#FFFF00";
			else if ($getStation_obj->sd_evn_edep <= 10) $color = "#00FF00";
			else if ($getStation_obj->sd_evn_edep <= 50) $color = "#0000FF";
			else $color = "#000080";
      ...
      map.addOverlay(marker);//hasdlfkjsf
