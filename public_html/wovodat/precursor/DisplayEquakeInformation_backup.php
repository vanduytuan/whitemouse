...
$sqlStatement1 ="(
                select b.sn_code, c.sd_evn_elat, c.sd_evn_elon, c.sd_evn_edep, c.sd_evn_pmag,
                UNIX_TIMESTAMP(c.sd_evn_time), c.sd_evn_eqtype, d.vd_inf_slat, d.vd_inf_slon
                FROM sn b, sd_evn c, vd_inf d WHERE b.sn_id = c.sn_id AND b.vd_id=d.vd_id
                AND d.vd_id = '$volcanoId' $dates $depth $equakeType ORDER BY (sd_evn_time) DESC limit $numberOfEvents)
                UNION (select b.sn_code, c.sd_evn_elat, c.sd_evn_elon, c.sd_evn_edep, c.sd_evn_pmag,
                c.sd_evn_time, c.sd_evn_eqtype, d.vd_inf_slat, d.vd_inf_slon
                FROM jj_volnet a, sn b, sd_evn c, vd_inf d WHERE a.vd_id = '$volcanoId'
                AND a.jj_net_id = b.sn_id AND b.sn_id = c.sn_id	AND d.vd_id = '$volcanoId'
                AND a.jj_net_flag = 'S' $dates $depth $equakeType
                AND power(d.vd_inf_slat - c.sd_evn_elat, 2) + power(d.vd_inf_slon - c.sd_evn_elon, 2)<=0.01
                ORDER BY (sd_evn_time) DESC limit $numberOfEvents)";
$sqlStatement = "select ";
$getQuakes = mysql_query($sqlStatement) or die('<b>Can\'t connect to server now!</b>');
...
while($row = mysql_fetch_array($getQuakes,MYSQL_NUM)) {
    array_push($eqtypes,$row[6]);
    echo "['" . $row[1] . "','" . $row[2] . "','" . $row[3] . "','" . $row[5] . "','" . $row[6] . "'],";
}
print_r($row);
sort($eqtypes);
...
$volcanoLatitude = $row[7];
$volcanoLongitude = $row[8];
mysql_data_seek($getQuakes,0);
